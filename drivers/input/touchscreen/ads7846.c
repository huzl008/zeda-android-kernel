...
static void ft5x0x_report_value(void)
{
	...
	{
		//meikee add
		//printk("-----x=%d---------y=%d------i=%d---\n",event->au16_x[i],event->au16_y[i],i);
		j=event->au16_x[i];
		event->au16_x[i]=(1024-j)*800/1024;
		j=event->au16_y[i];
		event->au16_y[i]=(600-j)*480/600;
...
static void ft5x0x_ts_pen_irq_work(struct work_struct *work)
{
	...
	enable_irq(gpio_to_irq(FT5X0X_GPIO_IRQ));
}
/***********************************************************************************************
Name	:	 
...
static irqreturn_t ft5x0x_ts_interrupt(int irq, void *dev_id)
{
	...
	disable_irq_nosync(gpio_to_irq(FT5X0X_GPIO_IRQ));
	//printk("-------------------------INT______\n");
						...
						const char *buf, size_t count)
{
	...
	u8 valbuf[5];memset(valbuf, 0, sizeof(valbuf));
	mutex_lock(&data->device_mode_mutex);
...
//	ssize_t num_read_chars = 0;
	...
	disable_irq(gpio_to_irq(FT5X0X_GPIO_IRQ));
						...
						const char *buf, size_t count)
{
	...
	data = (struct ft5x0x_ts_data *) i2c_get_clientdata( client );
//	ssize_t num_read_chars = 0;
	char fwname[128];memset(fwname, 0, sizeof(fwname));
...
ft5x0x_ts_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	err = gpio_request(FT5X0X_GPIO_RST, "ft5x0x reset");
	if (err < 0) {
		printk("--------faild request reset-----\n");
		}
	//s3c_gpio_setpull(FT5X0X_GPIO_RST, S3C_GPIO_PULL_UP);
	
	gpio_direction_output(FT5X0X_GPIO_RST,1);
	mdelay(20);
	gpio_direction_output(FT5X0X_GPIO_RST,0);
	mdelay(50);
	gpio_direction_output(FT5X0X_GPIO_RST,1);
	mdelay(20);

	err = gpio_request(FT5X0X_GPIO_IRQ, "TS_INT");	//Request IO
	if (err < 0) 
	{
		printk("faild to request   TS_INT\n");
	}
	//s3c_gpio_setpull(FT5X0X_GPIO_IRQ, S3C_GPIO_PULL_NONE);	//ret > 0 ?
	//s3c_gpio_cfgpin(FT5X0X_GPIO_IRQ, S3C_GPIO_SFN(3) );

	gpio_direction_input(FT5X0X_GPIO_IRQ);
	enable_irq(gpio_to_irq(FT5X0X_GPIO_IRQ));

	err = request_irq(gpio_to_irq(FT5X0X_GPIO_IRQ), ft5x0x_ts_interrupt, IRQF_TRIGGER_FALLING, "ft5x0x_ts", ft5x0x_ts);
	if (err < 0) {
...
//	free_irq(client->irq, ft5x0x_ts);
	free_irq(gpio_to_irq(FT5X0X_GPIO_IRQ), ft5x0x_ts);
