...
static struct ov5640_reg ov5640_vga_reg[] = {
	{0x3035, 0x11},
	{0x3036, 0x69},
	{0x3C07, 0x07},
	{0x3820, 0x41},
...
static struct ov5640_reg ov5640_5m_reg[] = {
	{0x3035, 0x21},
	{0x3036, 0x54},
	{0x3C07, 0x07},
	{0x3820, 0x40},
...
//	printk(KERN_DEBUG "%s() ENTER\n", __func__);
	 ...
	 * VGA resolution
	 */
	ret = ov5640_write_regs(client, ov5640_vga_reg);
	if (ret) {
...
fmt_found:
	 ...
	 */
	fse->min_width = fse->max_width = OV5640_DEF_WIDTH;
	fse->min_height = fse->max_height = OV5640_DEF_HEIGHT;
...
//	printk(KERN_DEBUG "%s() ENTER\n", __func__);
	...
	for (i = 0; i < ARRAY_SIZE(ov5640_fmt_list); i++) {
		if (fmt->format.code == ov5640_fmt_list[i].mbus_code )
			goto fmt_found;
	}
	if (i >= ARRAY_SIZE(ov5640_fmt_list))
		return -EINVAL;
...
fmt_found:
	 ...
	 */
	fmt->format.width = OV5640_DEF_WIDTH;
		...
		struct v4l2_subdev_fh *fh, struct v4l2_subdev_crop *crop)
{
	...
	rect.left = 0;
	rect.top = 0;
	rect.width = OV5640_DEF_WIDTH;
	rect.height = OV5640_DEF_HEIGHT;
...
static int ov5640_s_ctrl(struct v4l2_ctrl *ctrl)
{
	...
	struct i2c_client *client = v4l2_get_subdevdata(&ov5640->subdev);
